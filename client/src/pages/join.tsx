    ...
    <div className="page-container max-w-md">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="p-8 space-y-6 text-center">
          ...
          <div>
            <h1 className="text-2xl font-bold gradient-text">輸入房間代碼</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Input
              ...
              spellCheck={false}
              inputMode="text"
              className="h-16 text-center text-3xl font-mono tracking-[0.5em] uppercase"
            />
            <Button type="submit" disabled={loading || code.trim().length < 4} className="w-full h-12 text-lg" size="lg">
              {loading ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />查詢中…
                </>
              ) : (
                "進入投票"
              )}
          ...
          <p className="text-xs text-gray-400">代碼不分大小寫，且不會出現容易混淆的字元（0、O、1、I、L）</p>
        </Card>
      </motion.div>
    </div>
  );
