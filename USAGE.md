...
**修法**：在 `student.tsx` 加 `notFound` state，由 `listenToQuestion` 的 callback 區分：
- `q` 有值 → `setQuestion(q); setNotFound(false)`（正常顯示題目）
- `q === null` → `setNotFound(true)`（listener 已成功送出但題目不存在 → 流水號錯誤或題目已刪除）

Early return 拆兩條：
- 帶 ID + `notFound = true` → 「🔎 找不到這個題目，請跟老師確認連結是否正確，或重新掃 QR Code」+ 顯示 ID 方便對照
- 其他（沒帶 ID 直連根目錄、或還在載入中）→ 維持「目前沒有活動的問題」

檔案：`client/src/pages/student.tsx`
...
**模式切換** — 上方 toggle，`localStorage` 記住老師偏好（key: `teacher_form_mode`）：
- 💡 **引導**（預設）— 分 3 步建題
- ⚡ **快速** — 所有設定一次填完（資深者）

**Wizard 三步驟**：
1. **選題型 + 上傳圖片**（4 顆題型 toggle + ScreenshotUpload）
2. **設定選項**（單選/多選用，是非/簡答自動 disable）+ 具名 toggle
3. **摘要預覽**（縮圖、題型、具名狀態、選項列表）+「🚀 建立問題」

**進度條**：
- 圓點 + 連接線：已完成綠 ✓ / 目前藍 scale-110 + shadow / 未完成淺灰
- 手機版只顯示圓點，桌面版顯示步驟名稱

**導航按鈕**：
- step 1 沒圖片時「下一步」disabled
- step 2 沒滿 2 個選項時「下一步」disabled（單選/多選才檢查）
- step 3 是 form submit 觸發 `createQuestion`

**Quick 模式** 完全保留原本單頁布局（資深用戶不被打擾），題型 toggle 在 Card 內也只 quick 模式顯示（避免 wizard step 2 重複）。
...
**D5 課堂模式 QR 可放大縮小**：
- 4 段尺寸 S 200 / M 280 / L 360 / XL 460 px
- QR 卡片頂部加 ZoomOut / 尺寸字母 / ZoomIn 三鍵組
- `localStorage` 記住老師偏好（`present_qr_size`）
- 右側欄寬度動態跟著 QR 走（小教室不浪費空間、大禮堂不被截）

**A4 是非題（簡化版單選）**：
- `QuestionType` 加 `'truefalse'`，`TRUEFALSE_OPTIONS` 常數鎖定 `['⭕ 是', '❌ 否']`
- teacher 題型 toggle 變 3 顆：單選 / 多選 / ⚡是非題
- 選 truefalse 時選項輸入區自動 disable，amber 預覽卡片顯示兩個鎖定選項
- `createQuestion` 自動填 `TRUEFALSE_OPTIONS`，老師完全不用打字
- Student 投票 UI：grid-cols-2 並排 + aspect-square 超大按鈕、選項 0 綠、選項 1 紅、選中後 scale-105 + 2xl shadow

---

### 2026-05-04（下午 16 點） — D3 Skeleton loader ✅

把 dashboard 載入時的「Loader2 spinner + 載入題目中…」換成 6 張 `DashboardCardSkeleton` 網格，結構盡量貼近真實卡片：
- 縮圖區 4:3 + 左上 status badge 占位 + 右上 room code 占位
- 選項預覽 4 行 width 漸縮
- 票數 + 時間 row
- 按鈕列（主按鈕 + 兩個 icon button）

好處：
- 預載 layout 大小，內容載入時不會 layout shift（CLS ↓）
- 視覺上像「即將出現」而非「轉圈圈很久」，主觀載入速度感 ↑
- 重用 shadcn `<Skeleton>` component，零新依賴
